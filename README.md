...
- **Frontend:** `index.html` (স্টোর, হেডারে লগইন/ট্র্যাকিং/কার্ট আইকন-সহ) + `admin.html` (অ্যাডমিন প্যানেল) — Cloudflare Pages-এ হোস্ট হবে
...
তৈরি হয়। হোমপেজের হেডারে "ট্র্যাকিং" আইকনে ক্লিক করে **শুধু অর্ডার আইডি দিয়েই** (কোনো লগইন বা ফোন নাম্বার ছাড়াই)
অর্ডারের অবস্থা দেখা যায়। "লগইন" আইকনে ক্লিক করে ফোন নাম্বার + পাসওয়ার্ড দিয়ে অ্যাকাউন্ট বানিয়ে/লগইন করে গ্রাহক
তার সব অর্ডারের হিস্টোরি একসাথে দেখতে পারবেন, এবং পাসওয়ার্ড ভুলে গেলে শুধু ফোন নাম্বার দিয়েই নতুন পাসওয়ার্ড সেট
করা যাবে (কোনো OTP/ভেরিফিকেশন ছাড়া — এটা ইচ্ছাকৃতভাবে সহজ রাখা হয়েছে)।

> **নিরাপত্তা নোট:** যেহেতু ট্র্যাকিং এখন শুধু অর্ডার আইডি দিয়েই হয় (ফোন নাম্বার ছাড়া) এবং পাসওয়ার্ড রিসেট কোনো
> ভেরিফিকেশন ছাড়াই ফোন নাম্বার দিয়ে করা যায় — এই দুটোই ইচ্ছাকৃত সিদ্ধান্ত অনুযায়ী সরলীকৃত করা হয়েছে, কিন্তু
> এর মানে অর্ডার আইডি জানা যেকোনো ব্যক্তি সেই অর্ডারের ঠিকানা/নাম দেখতে পারবে, এবং কারো ফোন নাম্বার জানলে
> তার অ্যাকাউন্টের পাসওয়ার্ড বদলে দেওয়া সম্ভব। ভবিষ্যতে চাইলে SMS OTP যোগ করে এটা আরও নিরাপদ করা যাবে।

## ফাইল স্ট্রাকচার

```
├── index.html                     ← স্টোরফ্রন্ট (হোমপেজ, হেডারে লগইন/ট্র্যাকিং/কার্ট আইকন ও মডাল)
...
│       ├── orders/track.js        ← GET /api/orders/track?order_id=.. (পাবলিক ট্র্যাকিং, শুধু অর্ডার আইডি)
│       ├── admin/
│       │   ├── login.js           ← POST /api/admin/login
│       │   ├── logout.js          ← POST /api/admin/logout
│       │   └── orders.js          ← GET/PUT /api/admin/orders (অ্যাডমিন: সব অর্ডার দেখা ও স্ট্যাটাস বদলানো)
│       └── customer/
│           ├── register.js        ← POST /api/customer/register
│           ├── login.js           ← POST /api/customer/login
│           ├── logout.js          ← POST /api/customer/logout
│           ├── reset-password.js  ← POST /api/customer/reset-password (শুধু ফোন নাম্বার দিয়ে নতুন পাসওয়ার্ড সেট)
...
6. হোমপেজে হেডারের "ট্র্যাকিং" আইকনে ক্লিক করে সেই অর্ডার আইডি দিয়ে ট্র্যাক করে দেখুন
7. "লগইন" আইকনে ক্লিক করে একটা গ্রাহক অ্যাকাউন্ট বানিয়ে দেখুন সেই ফোন নাম্বারের অর্ডারগুলো হিস্টোরিতে আসছে কিনা
