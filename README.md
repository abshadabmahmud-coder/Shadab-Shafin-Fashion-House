...
├── admin.html                     ← অ্যাডমিন প্যানেল (এখন অর্ডার ম্যানেজমেন্টসহ)
├── order.html                     ← "আমার অর্ডার" পেজ — /order লিংকে খোলে (গ্রাহক লগ-ইন + অর্ডার হিস্টরি + ট্র্যাকিং)
├── _redirects                     ← Cloudflare Pages rewrite: /order → order.html
├── functions/
│   └── api/
│       ├── _utils.js              ← শেয়ার্ড হেল্পার (auth, পাসওয়ার্ড হ্যাশ, ফোন normalize ইত্যাদি)
...
│       ├── orders.js              ← POST /api/orders (চেকআউট করলে অর্ডার তৈরি হয়, পাবলিক)
│       ├── orders/track.js        ← GET /api/orders/track?id=..&phone=.. (পাবলিক, আইডি+ফোন মিলিয়ে ট্র্যাকিং)
│       ├── auth/
│       │   ├── register.js        ← POST /api/auth/register (গ্রাহক অ্যাকাউন্ট তৈরি)
│       │   ├── login.js           ← POST /api/auth/login (গ্রাহক লগ-ইন)
│       │   └── logout.js          ← POST /api/auth/logout
│       ├── customer/
│       │   └── orders.js          ← GET /api/customer/orders (লগ-ইন করা গ্রাহকের অর্ডার হিস্টরি)
│       └── admin/
│           ├── login.js           ← POST /api/admin/login
│           ├── logout.js          ← POST /api/admin/logout
│           ├── orders.js          ← GET /api/admin/orders (অ্যাডমিন — সব অর্ডার)
│           └── orders/[id].js     ← PATCH /api/admin/orders/:id (অ্যাডমিন — স্ট্যাটাস বদলানো)
├── schema.sql                     ← D1 টেবিল + ডিফল্ট ডেটা (products, settings, sessions, customers, customer_sessions, orders)
...
## নতুন ফিচার — অর্ডার হিস্টরি, ট্র্যাকিং ও গ্রাহক লগ-ইন

এই আপডেটে যোগ হয়েছে:

1. **`/order` পেজ** (`order.html`) — হোমপেজ থেকে আলাদা একটা প্যানেল, যেখানে গ্রাহক:
   - লগ-ইন/রেজিস্ট্রেশন করে তার সব অর্ডারের হিস্টরি দেখতে পারবে
   - অ্যাকাউন্ট ছাড়াই শুধু **অর্ডার আইডি + মোবাইল নাম্বার** দিয়ে যেকোনো অর্ডার ট্র্যাক করতে পারবে
2. **অর্ডার আইডি** — চেকআউট করলেই এখন সার্ভারে (D1) সত্যিকারের একটা ইউনিক অর্ডার তৈরি হয় (যেমন `SSFH-48213`) এবং সেই আইডি পরে ট্র্যাকিংয়ে ব্যবহার করা যায়। WhatsApp-ভিত্তিক অর্ডার পাঠানোর পুরনো সিস্টেমও অপরিবর্তিত আছে — শুধু এখন D1-এও রেকর্ড থাকে।
3. **গ্রাহক লগ-ইন সিস্টেম** — মোবাইল নাম্বার + পাসওয়ার্ড দিয়ে অ্যাকাউন্ট, পাসওয়ার্ড D1-এ কখনো প্লেইন টেক্সটে সেভ হয় না (PBKDF2 হ্যাশ + salt)। লগ-ইন করা অবস্থায় চেকআউট করলে অর্ডারটি স্বয়ংক্রিয়ভাবে সেই অ্যাকাউন্টের সাথে যুক্ত হয়ে যায়। গেস্ট অর্ডারও চলবে — পরে একই মোবাইল নাম্বার দিয়ে অ্যাকাউন্ট খুললে সেই পুরনো অর্ডারগুলোও হিস্টরিতে দেখা যাবে (ফোন নাম্বার দিয়ে ম্যাচ করা হয়)।
4. **অ্যাডমিন প্যানেলে অর্ডার ম্যানেজমেন্ট** — `admin.html`-এ এখন একটা "অর্ডার তালিকা" সেকশন আছে, যেখানে সব অর্ডার দেখা যায় এবং প্রতিটার স্ট্যাটাস (অপেক্ষমান → নিশ্চিত → প্রস্তুত হচ্ছে → পাঠানো হয়েছে → ডেলিভারি সম্পন্ন / বাতিল) বদলানো যায়। স্ট্যাটাস বদলালে সাথে সাথে গ্রাহকের ট্র্যাকিং/অর্ডার হিস্টরিতে নতুন স্ট্যাটাস দেখা যাবে।

**গুরুত্বপূর্ণ:** এই ফিচার কাজ করার জন্য `schema.sql` আবার রান করতে হবে (নতুন `customers`, `customer_sessions`, `orders` টেবিল যোগ হয়েছে) — নিচের ধাপ ৪ দেখুন। আগে থেকে ডিপ্লয় করা থাকলে শুধু নতুন টেবিলগুলোর `CREATE TABLE IF NOT EXISTS` অংশটুকু D1 Console-এ রান করলেই হবে, পুরনো ডেটা অক্ষত থাকবে।

`/order` URL কাজ করার জন্য `_redirects` ফাইলটা রুট ফোল্ডারেই (index.html-এর পাশে) থাকতে হবে — এটা Cloudflare Pages-কে বলে দেয় যে `/order` রিকোয়েস্ট এলে `order.html` দেখাতে হবে।
